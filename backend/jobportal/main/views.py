...
def activate_user(request, token):
    try:
        ...
        payload = signer.unsign(decoded, max_age=86400)
        data = json.loads(payload)
        role = data.get('role', 'employer')

        if User.objects.filter(email=data['email']).exists():
            redirect_url = '/signin?verified=false' if role == 'employer' else '/applicantlogin?verified=false'
            return Response({
                'success': False,
        ...
        # ROLE-BASED REDIRECT
        redirect_url = '/signin?verified=true&role=employer' if role == 'employer' else '/applicantlogin?verified=true&role=applicant'
        ...
        role = 'employer'  # fallback
        return Response({
            'success': False,
            'message': 'Link expired. Please sign up again.',
            'redirect': '/signup' if role == 'employer' else '/applicantsignup'
        }, status=status.HTTP_400_BAD_REQUEST)
    except BadSignature:
        role = 'employer'
        return Response({
